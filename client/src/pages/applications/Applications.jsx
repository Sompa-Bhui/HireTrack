...
const sourceOptions = ['LinkedIn', 'Indeed', 'Naukri', 'Foundit', 'Wellfound', 'HR Email', 'Company Career Page', 'Referral', 'Internshala', 'Glassdoor', 'Other'];
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 md:grid-cols-2">
              ...
              <select className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none ring-0" {...register('applicationSource')}>
                {sourceOptions.map((item) => <option key={item} value={item}>{item}</option>)}
              </select>
              <select className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none ring-0" {...register('status')}>
  ...
  return (
    <div className="relative space-y-5 pb-28">
      ...
      <div className="grid gap-4">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div className="space-y-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {item.archived ? <Badge className="border-amber-500/30 bg-amber-500/15 text-amber-200">Archived</Badge> : null}
                </div>
              ...
              <div className="flex flex-wrap gap-2 lg:justify-end">
                ...
                <button className="rounded-xl bg-white/5 px-3 py-2 text-sm" onClick={() => archiveMutation.mutate(item._id)}>{item.archived ? 'Archived' : 'Archive'}</button>
        ...
        {!isLoading && !items.length && <div className="rounded-3xl border border-dashed border-white/10 bg-white/5 p-10 text-center text-white/60">No applications yet.</div>}
      </div>
